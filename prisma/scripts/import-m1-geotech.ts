...
const CSV_FILE = path.join(process.cwd(), 'prisma/students/master_un_geotchenique.csv')
...
async function main() {
    console.log("🚀 Importation des étudiants Master 1 Géotechnique...");
    ...
    // Niveau M1 Geotechnique (ID 6 selon levelMapping dans import-courses.ts)
    const levelId = 6;
    const levelCode = 'm1_geotechnique';
    ...
    const academicLevel = await prisma.academicLevel.upsert({
        ...
        create: {
            ...
            displayName: 'Master 1 (Géotechnique) / Géologie',
            order: 6,
            isActive: true
        }
    });
