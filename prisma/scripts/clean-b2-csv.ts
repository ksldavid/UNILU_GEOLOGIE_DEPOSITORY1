...
const CSV_FILE = path.join(process.cwd(), 'prisma/students/Bac_deux.csv');
...
async function cleanCSV() {
    console.log("🧹 Début du nettoyage du fichier BAC 2 (Bac_deux.csv)...");
    ...
    for (const record of records) {
        ...
        const studentID = record['ID']?.trim(); // Dans ce fichier c'est "ID"
    ...
    if (cleaned.length > 0) {
        ...
        console.log(`✅ Nettoyage BAC 2 terminé ! ${removedCount} lignes traitées.`);
    }
}
