...
export function AttendanceManagement({ course, onBack, onDirtyChange, saveTrigger }: AttendanceManagementProps) {
  ...
  // Modal liste étudiants inscrits
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto animate-in fade-in duration-300">
        ...
        <>
          ...
          <div className="bg-gradient-to-br from-teal-50 to-cyan-50 border border-teal-200 rounded-2xl p-8 mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              <button
                onClick={() => { setShowEnrolledModal(true); setEnrolledSearchQuery(""); }}
                className="text-left bg-white rounded-xl p-6 border border-gray-200 shadow-sm hover:border-teal-400 hover:shadow-md transition-all group cursor-pointer active:scale-95"
                title="Cliquer pour voir la liste des étudiants"
              >
                <div className="text-3xl font-bold text-gray-900 mb-1 group-hover:text-teal-600 transition-colors">
                  {students.length}
                </div>
                <div className="text-gray-500 font-bold uppercase tracking-wider text-[10px] flex items-center gap-1">
                  Total inscrits
                  <Users className="w-3 h-3 text-teal-400 opacity-0 group-hover:opacity-100 transition-opacity" />
                </div>
              </button>
      ...
      {/* Modal Liste Étudiants Inscrits (Total Inscrits click) */}
      {showEnrolledModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4 animate-in fade-in zoom-in duration-200" onClick={(e) => { if (e.target === e.currentTarget) setShowEnrolledModal(false); }}>
          <div className="bg-white rounded-[32px] shadow-2xl max-w-2xl w-full h-[85vh] overflow-hidden relative border border-gray-100 flex flex-col">
            {/* Header */}
            <div className="p-6 border-b border-gray-100 bg-gray-50/50 flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h2 className="text-xl font-bold text-gray-900">Liste des Étudiants Inscrits</h2>
                  <p className="text-xs font-semibold text-gray-400 uppercase tracking-widest">{course.name} · {students.length} étudiant(s)</p>
                </div>
              </div>
              <button
            ...
            {/* Stats rapides */}
            <div className="grid grid-cols-3 gap-3 p-4 bg-white border-b border-gray-100">
              <div className="bg-emerald-50 border border-emerald-100 p-3 rounded-xl text-center">
                <div className="text-xl font-black text-emerald-700">{Object.values(selectedStatus).filter(s => s === 'present').length}</div>
                <div className="text-[9px] font-black text-emerald-600 uppercase tracking-widest">Présents</div>
              </div>
              <div className="bg-amber-50 border border-amber-100 p-3 rounded-xl text-center">
                <div className="text-xl font-black text-amber-700">{Object.values(selectedStatus).filter(s => s === 'late').length}</div>
                <div className="text-[9px] font-black text-amber-600 uppercase tracking-widest">En retard</div>
              </div>
              <div className="bg-red-50 border border-red-100 p-3 rounded-xl text-center">
                <div className="text-xl font-black text-red-700">{students.length - Object.values(selectedStatus).filter(s => s === 'present' || s === 'late').length}</div>
                <div className="text-[9px] font-black text-red-600 uppercase tracking-widest">Absents</div>
              </div>
            </div>
            ...
            <div className="px-5 py-3 border-b border-gray-100">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-4 py-2 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-transparent"
                />
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto">
              <table className="w-full border-separate border-spacing-y-1 px-4 pt-2 pb-4">
                ...
                <tbody>
                  {students
                    .filter(s =>
                      ...
                      s.id.toLowerCase().includes(enrolledSearchQuery.toLowerCase())
                    )
                    .sort((a, b) => a.name.localeCompare(b.name))
                    .map((student, index) => {
                      const status = selectedStatus[student.id];
                      const isPresent = status === 'present';
                      const isLate = status === 'late';
                      const isAbsent = !isPresent && !isLate;
                      return (
                        <tr key={student.id} className={`transition-colors group ${isPresent ? 'bg-emerald-50/60 hover:bg-emerald-50' :
                            isLate ? 'bg-amber-50/60 hover:bg-amber-50' :
                          ...
                          <td className="px-3 py-2">
                            <span className="font-bold text-gray-900 text-sm group-hover:text-teal-700 transition-colors">{student.name}</span>
                          </td>
                          <td className="px-3 py-2">
                            <span className="font-mono text-gray-400 text-xs">{student.id}</span>
                          </td>
                          <td className="px-3 py-2 text-center rounded-r-xl">
                            <span className={`inline-flex items-center gap-1 px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest ${isPresent ? 'bg-emerald-100 text-emerald-700' :
            ...
            <div className="p-4 border-t border-gray-100 bg-gray-50/50">
              <button
                onClick={() => setShowEnrolledModal(false)}
                className="w-full py-3 bg-gray-900 hover:bg-black text-white rounded-xl font-bold text-sm transition-all shadow-lg active:scale-[0.98]"
              >
                Fermer
              </button>
            </div>
