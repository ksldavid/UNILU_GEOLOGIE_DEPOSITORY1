...
const CSV_FILE = path.join(process.cwd(), 'prisma/students/master_deux_exploration_miniere.csv')
...
async function main() {
    console.log("🚀 Importation des étudiants Master 2 Exploration...");
    ...
    // Niveau M2 Exploration (ID 8 selon levelMapping dans import-courses.ts)
    const levelId = 8;
    const levelCode = 'm2_exploration';
    ...
    const academicLevel = await prisma.academicLevel.upsert({
        ...
        create: {
            id: levelId,
            code: levelCode,
            name: 'Master 2 (Exploration)',
            displayName: 'Master 2 (Exploration) / Géologie',
            order: 8,
            isActive: true
        }
    });
